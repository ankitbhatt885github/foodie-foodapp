...
const Cart = () => {
  const navigate = useNavigate();
    ...
    //to toggle the cart menu on and off
    const [activeCart,setActiveCart] = useState(false);

    


    //select the cart array in slice that is inside the Store
  ...
  const totalPrice = cartItems.reduce(
    ...
    return (
        <>
          ...
          activeCart ? "translate-x-0" : "translate-x-full"
                ...
                <div className="absolute bottom-0 ">
                    ...
                    <button onClick={() => {
                      navigate("/success")
                    }} className="bg-sky-500/100 font-bold px-3 text-white py-2 rounded-lg w-[90vw] lg:w-[18vw] mb-5">Checkout</button>
        ...
        className={`rounded-full bg-white shadow-md text-sky-400 text-5xl p-3 fixed bottom-4 right-4 ${
